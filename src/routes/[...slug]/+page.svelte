<script lang="ts">
	...
	onMount(() => {
		...
		console.log('Catch-all route hit:', currentPath);
		console.log('Hash:', hash);
		
		// Check if this is the PocketBase verification URL pattern
		if (currentPath === '//_' || currentPath.includes('//_/')) {
			if (hash) {
		...
		// If this is not a known pattern, redirect to home
...
<div class="min-h-screen flex items-center justify-center bg-gray-50">
	<div class="max-w-md w-full text-center">
		...
		<p class="text-gray-600">Processing your link...</p>
		<p class="text-sm text-gray-500 mt-4">Path: {$page.url.pathname}</p>
	</div>
</div>
